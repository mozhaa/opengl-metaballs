...
void main() {
    ...
    float cosine = dot(normalize(normal), normalize(sun_direction));
    float light_factor = max(0.0, cosine);
    vec3 reflect_dir = 2.0 * normalize(normal) * cosine - normalize(sun_direction);
    
    vec3 view_dir = normalize(camera_position - position);

    vec3 ambient_light = albedo * ambient_strength;
    vec3 diffuse_light = sun_color * albedo * light_factor;
    vec3 specular_light = specular_strength * sun_color * pow(max(0.0, dot(normalize(reflect_dir), view_dir)), power);
